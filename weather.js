    ...
    if(index === 0) { // HTML for the main weather card
        return `<div class="details">
                ...
                <div class="icon">
                    ...
                    <h4>Weather description: ${weatherItem.weather[0].description}</h4>
                </div>`;
...
const getWeatherDetails = (cityName, latitude, longitude, country) => {
    ...
    fetch(WEATHER_API_URL).then(response => response.json()).then(data => {
        ...
        fiveDaysForecast.forEach((weatherItem, index) => {
            const forcast = createWeatherCard(cityName, weatherItem, index, country);
            if (index === 0) {
                currentWeatherDiv.insertAdjacentHTML("beforeend", forcast);
            } else {
                weatherCardsDiv.insertAdjacentHTML("beforeend", forcast);
            }
        });        
    }).catch(() => {
